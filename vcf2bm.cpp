...
void vcf_to_binmat(const char* vcf_path, const char* out_path,
    ...
    std::cerr << "Start parsing vcf, will read " << total_runs + 1
    ...
    for (size_t run = 0; run <= total_runs; ++run) {
        ...
        while (std::getline(vcf_file, line)) {
            ...
            auto split_line =
                split_string(line, '\t', individual_count + offset);
            ...
            for (size_t idx = run * at_once; idx < (run + 1) * at_once; ++idx) {
                ...
                if (split_line[id].size() > 3)
                    std::cerr << "Got overly long haplotype in run " << run
                              << " for individual " << individual_id << ": "
                              << split_line[id] << std::endl;

                try {
                    // chromosome 1 of individual
                    bin_rows[2 * individual_id]->push_back(
                        split_line[id].at(0) != '0');
                    // chromosome 2 of individual
                    bin_rows[2 * individual_id + 1]->push_back(
                        split_line[id].at(2) != '0');
                    individual_id++;
                } catch (const std::out_of_range& e) {
                    std::cerr << e.what() << std::endl;
                    std::cerr << "Thrown when trying to parse "
                              << split_line[id] << "."
                              << " Possibly not diploid?" << std::endl;
                    std::cerr << line << std::endl;
                    std::cerr
                        << "Individual: " << individual_id
                        << ", line: " << bin_rows[2 * individual_id]->size()
                        << std::endl;
                    exit(EXIT_SUCCESS);
                }
            }
        ...
        for (auto ptr : bin_rows) {
            ...
            for (bool b : *ptr) out_file.write(b ? "1" : "0", 1);
            out_file.write("\n", 1);
            ptr->clear();
        }
        out_file.close();
        ...
        std::cerr << "Run " << run + 1 << " of " << total_runs + 1
                  << " completed" << std::endl;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (has_cmd_option(argv, argv + argc, "-h") ||
        ...
        std::cout
            << "Call with <prog> -v vcf_file -o out_file [-a at_once] [-h]"
            << std::endl;
        return EXIT_FAILURE;
    }
